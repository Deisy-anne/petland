...
export class Pet {
  ...
  static create ({ id, name, age, userId, weight, birthDate }: Pet.Input): Pet.Output {
    ...
    if (birthDate !== undefined && birthDate !== null && !this.isValidBirthDate(birthDate)) errors.push(InvalidPetBirthDateError(birthDate))
  ...
  static isValidBirthDate (birthDate: Date): boolean {
    return DateUtils.isDefined(birthDate)
  }
}
